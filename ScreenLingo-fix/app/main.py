...
from PySide6.QtGui import QColor, QPainter, QFont, QFontMetrics, QPen
from PySide6.QtWidgets import (QApplication, QWidget, QVBoxLayout, QHBoxLayout,
    QLabel, QPushButton, QComboBox, QSlider, QMessageBox, QFrame)
from models import ROOT, DATA, required, find_model
...
class Overlay(QWidget):
    ...
    def paintEvent(self, event):
        ...
        font = QFont('Malgun Gothic', self.font_size)
        ...
        for line in self.lines:
            ...
            width = min(max(w+20, 130), max(130, self.width()-16), 650)
            x = max(4, min(x-4, self.width()-width-4))
            text_rect = metrics.boundingRect(QRect(0,0,int(width)-16,2000), Qt.TextWordWrap, line['text'])
            height = max(h+8, text_rect.height()+12)
            height = min(height, self.height()-8)
            y = max(4, min(y-3, self.height()-height-4))
            box = QRectF(x,y,width,height)
            for earlier in occupied:
                if box.intersects(earlier):
                    box.moveTop(earlier.bottom()+3)
            if box.bottom() > self.height()-4:
                continue
            occupied.append(box)
            painter.setPen(QPen(QColor('#527869'), 1))
            painter.setBrush(QColor(12, 25, 29, 240))
            painter.drawRoundedRect(box, 5, 5)
            painter.setPen(QColor('#f0fff8'))
            painter.drawText(box.adjusted(8,5,-8,-5), Qt.TextWordWrap, line['text'])
...
class Panel(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('ScreenLingo · 화면 한국어 번역')
        ...
        self.worker_warning = ''
        self.last_frame = 0
        self.config = {'source':'en','interval':1200,'font':15,'region':None,'consent':False}
        if not PREVIEW:
            try:
        ...
        self.label(layout,'SCREENLINGO  /  0.1 BETA','brand')
        ...
        self.button(layout,'전체 종료',self.close,'danger')
        privacy = self.label(layout,'화면·글자 외부 전송 없음 · 캡처 파일 저장 없음\n창을 닫으면 화면 인식도 완전히 종료됩니다.','muted')
        privacy.setWordWrap(True)
        self.label(layout,'Ctrl+Alt+F8 일시정지   F9 번역 표시   F10 종료','muted')
    ...
    def start(self):
        ...
        if any(not find_model(pair) for pair in required(source)):
    ...
    def make_process(self, script, source):
        ...
        process.setArguments(['-B','-u',str(ROOT/'app'/script),source])
    ...
    def read_process(self, process):
        ...
        while b'\n' in buffer:
            ...
            if kind == 'ready' and self.running:
                ...
                self.busy = False; self.last_frame = time.monotonic()
                self.signature = data['signature']; self.overlay.lines = data['lines']; self.overlay.update()
                self.status.setText(f"● 번역 중 · {len(data['lines'])}줄 · 최근 처리 {data['seconds']}초" + self.worker_warning +
                    ('\n글자가 많아 60줄까지만 표시합니다. 영역을 좁혀 주세요.' if data.get('limited') else ''))
            elif kind == 'progress':
        ...
        if process in self.buffers:
            self.buffers[process] = buffer[-1_000_000:]

    def request_capture(self):
        if not self.running or self.busy or not self.worker or self.worker.state() != QProcess.Running:
            return
        self.busy = True
        packet = {'type':'capture','region':self.config['region']}
        self.worker.write((json.dumps(packet)+'\n').encode())

    def expire_stale(self):
        if self.last_frame and time.monotonic()-self.last_frame > 8:
            self.overlay.clear()
